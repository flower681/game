<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:89:"E:\wamp\phpStudy\PHPTutorial\WWW\game\public/../application/admin\view\index\chatOne.html";i:1560165236;}*/ ?>
<html><head>
	...
	<script type="text/javascript">
		...
		function onopen()
		{
			$.ajax({
				type:'get',
				url:'<?php echo url("admin/index/user"); ?>',
				success:function(res){
					// console.log(res);
					name = res.username;
					user_id = res.id;
					// 登录
					var login_data = '{"type":"login","user_id":"'+user_id+'","client_name":"'+name.replace(/"/g, '\\"')+'","room_id":"<?php echo isset($_GET['room_id']) ? $_GET['room_id'] : 1?>"}';
		...
		function onmessage(e)
		{
			...
			switch(data['type']){
				...
				// 登录 更新用户列表
				case 'login':
					//发送ajax请求，将用户信息保存到数据库中
					$.ajax({
						type:'post',
						url:'<?php echo url("admin/index/send"); ?>',
						data:data,
						success:function(res){
							console.log(res);
					...
					$.ajax({
						...
						success:function(res){
							...
							console.log(res);
							$('.say-btn').append('<input type="hidden" id="client_id" value="'+res['id']+'">');
							// say(res['id'], res['username'],  res['username']+' 加入了聊天室', res['time']);
							...
							// else
							// {
							// 	client_list[res['id']] = res['username'];
							// }
							// flush_client_list();
					...
					// $('.say-btn').append('<input type="hidden" id="client_id" value="'+data['client_id']+'">');
					//{"type":"login","client_id":xxx,"client_name":"xxx","client_list":"[...]","time":"xxx"}
					// say(data['client_id'], data['client_name'],data['to_client_id'],  data['client_name']+' 加入了聊天室', data['time']);
					if(data['client_list'])
					{
				...
				case 'say':
					...
					say(data['from_client_id'], data['from_client_name'], data['to_client_id'],getCaption(data['content']), data['time']);
					break;
				// 用户退出 更新用户列表
				case 'logout':
					//{"type":"logout","client_id":xxx,"time":"xxx"}
					// say(data['from_client_id'], data['from_client_name'], data['to_client_id'],data['from_client_name']+' 退出了', data['time']);
		...
		function onSubmit() {
			...
			var fname = $('#hidden').val();
			var from_client_id = $('#client_id').val();
			// console.log(fname);
			...
			var data = {type:"say",to_client_id:to_client_id,from_client_id:from_client_id,fname:fname,to_client_name:to_client_name,content:input.value.replace(/g, /).replace(/\n/g,'\\n').replace(/\r/g, '\\n')};
			console.log(data);
			$.ajax({
				type:'post',
				data:data,
				url:'<?php echo url("admin/index/msg"); ?>',
		...
		// 发言
		function say(from_client_id, from_client_name, to_client_id,content, time){
			// console.log(content);
			//解析新浪微博图片
		...
		$(function(){
			...
			$("#client_list").change(function(){
				...
				if(select_client_id == '0')
				{
					$("#dialog").append('<div class="speech_item">'+username+' <br> '+time+'<div style="clear:both;"></div><p class="triangle-isosceles top">'+username+'上线</p> </div>').parseEmotion();
				}
			// <img src="http://lorempixel.com/38/38/?" class="user_icon" />
					setInterval('getOneMsg(from_client_id,select_client_id)', 1000);
				// getOneMsg(from_client_id,select_client_id);
				
				//发送ajax请求，获取聊天数据
				// $.ajax({
				// 	type:'get',
				// 	data:{select_client_id:select_client_id,from_client_id:from_client_id},
				// 	url:'<?php echo url("admin/index/getOneMsg"); ?>',
				// 	success:function(ret){
				// 		// console.log(ret);
				...
				// 		{
				//
				// 			$("#dialog").append('<div class="speech_item"><img src="http://lorempixel.com/38/38/?'+ret[i]["fuid"]+'" class="user_icon" /> '+ret[i]["fname"]+' <br> '+ret[i]["send_time"]+'<div style="clear:both;"></div><p class="triangle-isosceles top">'+ret[i]["content"]+'</p> </div>').parseEmotion();
				// 		}
				// 	}
				// });
		...
		function getOneMsg(from_client_id,select_client_id)
		{
			$.ajax({
				type:'get',
				data:{select_client_id:select_client_id,from_client_id:from_client_id},
				...
				success:function(ret){
					...
					for(var i=0;i<ret.length;i++)
					{
						...
						$("#dialog").append('<div class="speech_item">'+ret[i]["fname"]+' <br> '+ret[i]["send_time"]+'<div style="clear:both;"></div><p class="triangle-isosceles top">'+ret[i]["content"]+'</p> </div>').parseEmotion();
					}
				// <img src="http://lorempixel.com/38/38/?'+ret[i]["fuid"]+'" class="user_icon" />
				}
			});
		}
...
<div class="container" >
	<div class="row clearfix">
			...
			<div class="thumbnail">
				<div class="caption" id="dialog">
					<div class="speech_item">
						<!--<img src="http://lorempixel.com/38/38/?'<?php echo $data['id']?>'" class="user_icon" /> -->
						<?php echo $data['username']?><br><?php echo $data['time']?>
						<div style="clear:both;">
						</div>
						<p class="triangle-isosceles top"><?php echo $data['username']?>上线</p>
					</div>
				</div>
			</div>
		...
		<!--<div class="col-md-3 column">-->
			...
			<!--</div>-->
			<!--&lt;!&ndash;<a href="http://workerman.net:8383" target="_blank"><img style="width:252px;margin-left:5px;" src="/static/Web/img/workerman-todpole.png"></a>&ndash;&gt;-->
		<!--</div>-->
	</div>
</div>
